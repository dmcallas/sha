 ...
 * @param pad_byte_arr Padded output.
 */
void pad1024(const uint64_t len, uint8_t* byte_arr, uint64_t* pad_len, uint8_t* pad_byte_arr){
  /*
   * Calculate padding.
   * start pad is always 1 byte (0b10000000).
   * end pad is always 16 bytes (128-bits).
   * Require multiple of 128 bytes at the end.
   */
  uint64_t base_pad_len   = len + 17;
  uint64_t zero_pad_amt   = 128 - (base_pad_len % 128);
  uint64_t full_len_bytes = base_pad_len + zero_pad_amt;
  //uint64_t full_len_bits  = full_len_bytes * 8;
  uint64_t len_bits       = len * 8;
  *pad_len = full_len_bytes;
